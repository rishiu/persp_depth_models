...
class SD_Dataset(Dataset):
    def __init__(self, data_dir, config, mode, transform=None):
        self.data_dir = data_dir
        folder = 'val'
        if mode == 'train':
            folder = 'train'
        #self.paths = pd.read_csv(csv_file, header=None, names=['image', 'depth'])
        self.rgb_paths = natsorted(glob(f"{self.data_dir}/{folder}/imgs/*.jpg"))
        self.depth_paths = natsorted(glob(f"{self.data_dir}/{folder}/depth/*.npy"))
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            #image, depth_gt, depth_completed = self.random_crop(image, depth_gt, depth_gt, self.input_height, self.input_width)
            image, depth_gt, depth_completed = self.train_preprocess(image, depth_gt, depth_gt)
        ...
        if self.mode != 'train':
            ...
            sample['path'] = os.path.basename(self.rgb_paths[idx]) #) #.replace("_colors", "")
...
class SD_DataModule(LightningDataModule):
    def __init__(self, config: DictConfig):
        ...
        mode = "train"
        self.training_samples = SD_Dataset(config.DATASET.PATH, self.config, mode, transform=preprocessing_transforms(mode))
        ...
        mode = "test"
        self.testing_samples = SD_Dataset(config.DATASET.PATH, self.config, mode, transform=preprocessing_transforms(mode))
