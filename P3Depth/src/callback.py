    ...
    def on_validation_epoch_end(self, trainer, lit_model):
        ...
        if trainer.logger is not None:
            ...
            if flag_offset:
                ...
                dt = np.random.randint(0, self.num_samples)
                rgb_grid = make_grid(rgb[dt], nrow=1)
                dgt_grid = make_grid(depth_gt[dt], nrow=1)
                dfinal_grid = make_grid(depth_final_prediction[dt], nrow=1)
                offset_grid = make_grid(offset_prediction[dt], nrow=1)

                trainer.logger.experiment.add_image("RGB", rgb_grid, 0)
                trainer.logger.experiment.add_image("Depth GT", dgt_grid, 0)
                trainer.logger.experiment.add_image("Depth Final", dfinal_grid, 0)
                trainer.logger.experiment.add_image("Offset", offset_grid, 0)
