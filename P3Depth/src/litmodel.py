...
class DepthLitModel(LightningModule):
    ...
    def standardize(self, arr):
        ...
        arr = (arr - a_min) / (a_max - a_min + 0.001) + 0.001
        return arr
    ...
    def training_step(self, batch, batch_idx):
        ...
        depth = self.standardize(depth)
        depth_completed = self.standardize(depth_completed)

        output_dict["depth_init", 1] = self.standardize(output_dict["depth_init", 1])
        output_dict["depth_final", 1] = self.standardize(output_dict["depth_final",1])
