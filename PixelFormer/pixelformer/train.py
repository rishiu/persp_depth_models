...
parser.add_argument('--data_file',                 type=str, help='path to training data')
...
elif args.dataset == "vkitti" or args.dataset == "both":
    from dataloaders.dataloader_syndata import NewDataLoader
from dataloaders.dataloader_diode import NewDataLoaderV2
...
def online_eval(model, dataloader_eval, gpu, ngpus, post_process=False):
    ...
    for _, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        ...
        measures = torch.tensor(measures).cuda(device=gpu)

        if not torch.isfinite(measures).all():
            continue
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.checkpoint_path != '':
        if os.path.isfile(args.checkpoint_path):
            ...
            print("== Loaded checkpoint '{}' (global_step)".format(args.checkpoint_path))
        else:
    ...
    dataloader_eval = NewDataLoaderV2(args, 'online_eval')
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            scale, shift = compute_scale_and_shift(depth_est.squeeze(1), depth_gt.squeeze(1), mask.squeeze(1))

            #print(torch.isfinite(scale).all())
            #print(torch.isfinite(shift).all())

            est_np = depth_est.clone().detach().cpu().numpy()
            est_np = est_np[0,0,:,:]

            gt_np = depth_gt.clone().detach().cpu().numpy()
            gt_np = gt_np[0,0,:,:]

            #fig, ax = plt.subplots(2)
            #eim = ax[0].imshow(est_np)
            #gim = ax[1].imshow(gt_np)
            #plt.colorbar(eim)
            #plt.colorbar(gim)
            #im = plt.imshow(est_np)
            
            #plt.colorbar(im)
            #plt.savefig(f"./test{step}.jpg")
            #plt.close()

            depth_est = scale.view(-1,1,1,1) * depth_est + shift.view(-1,1,1,1)

            #print(torch.isfinite(depth_est).all())

            mask = mask & (depth_est > 0.0)
