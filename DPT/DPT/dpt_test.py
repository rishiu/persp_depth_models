...
class DPTData(Dataset):
  ...
  def __init__(self, root_dir_list, is_train=True):
    ...
    for root_dir in root_dir_list:
      ...
      self.depth_paths += natsorted(glob(f"{root_dir}/depths/*.h5"))
  ...
  def __getitem__(self, index):
    ...
    depth_img = depth_file.get('depth')[:,:]
    ...
    inp_img = cv2.resize(inp_img, (new_h, new_w), interpolation=cv2.INTER_NEAREST)
    depth_img = cv2.resize(depth_img, (new_h, new_w), interpolation=cv2.INTER_NEAREST)
...
params = {
  ...
  'model_path': './checkpoints/syn_data_base/model_epoch_95.pth', # Dir to load model weights
  ...
  'test': True,
  'test_root_dir': ['/usr/project/depth_models/depth_data/phoenix/S6/zl548/MegaDepth_v1/0016/dense0']
}
...
test_dataset = DPTData(
  root_dir_list=params["test_root_dir"],
  is_train=False
)
...
if params['resume_train'] or params['test']:
...
with torch.no_grad():
  for batch_idx, batch_data in enumerate(tqdm(test_dataloader)):
    ...
    out = standardize(out)
    depth_img = standardize(depth_img)
    ...
    plt.figure()
    plt.axis('off')
    inp = plt.imshow(input_img[0, :, :, :].cpu().permute(1, 2, 0) * .5 + .5)
    plt.savefig("test_outputs/base_img_"+str(batch_idx)+".jpg", bbox_inches='tight')
    plt.close()

    min_v = torch.min(depth_img_plot)
    max_v = torch.max(depth_img_plot)
    plt.figure()
    plt.axis('off')
    outp = plt.imshow(out_plot, cmap = 'jet')
    plt.savefig("test_outputs/base_out_"+str(batch_idx)+".jpg", bbox_inches='tight')
    plt.close()
    ...
    plt.figure()
    plt.axis('off')
    depthp = plt.imshow(depth_img_plot, cmap = 'jet')
    #plt.colorbar(inp)
    #plt.colorbar(depthp)
    plt.savefig("test_outputs/base_gt_"+str(batch_idx)+".jpg", bbox_inches='tight')
    plt.close()


    # diff = torch.square(out.flatten(start_dim=1) - depth_img.flatten(start_dim=1)) / depth_img.flatten(start_dim=1)
    # #print(diff[0])
    # diff = torch.nan_to_num(diff, posinf=-1, neginf=-1)
    # N = torch.count_nonzero(diff > 0.0, dim=1)
    # loss = torch.mean(torch.sum(diff, axis=1) / N)

    absrel = abs_rel(out, depth_img)
    sqrel = sq_rel(out, depth_img)
    rmse_l = rmse(out, depth_img)
    lrmse = log_rmse(out, depth_img)

    overall_sqrel += sqrel.item()
    overall_absrel += absrel.item()
    overall_rmse += rmse_l.item()
    overall_lrmse += lrmse.item()
      
    # Log loss
    #writer.add_scalar('loss/train', loss_log, batch_idx)

    # if params['masked_ssi_loss_weight']:
    #   writer.add_scalar('masked_ssi_loss/train', loss_ssi_masked_log, batch_idx)
    # if params['smooth_loss_weight']:
    #   writer.add_scalar('smooth_loss/train', loss_smooth_log, batch_idx)
    # if params['ssi_trim_loss_weight']:
    #   writer.add_scalar('ssi_trim_loss/train', loss_ssi_trim, batch_idx)
    # if params['scale_inv_grad_loss_weight']:
    #   writer.add_scalar('scale_inv_grad_loss/train', loss_ssi_trim, batch_idx)

    count += 1

overall_sqrel /= count
overall_absrel /= count
overall_rmse /= count
overall_lrmse /= count
      

print("Overall Sq Rel: " + str(overall_sqrel))
print("Overall Abs Rel: " + str(overall_absrel))
print("Overall RMSE: " + str(overall_rmse))
print("Overall Log RMSE: " + str(overall_lrmse))
