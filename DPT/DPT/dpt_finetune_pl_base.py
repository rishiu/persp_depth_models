...
params = {
  ...
  'save_dir': './checkpoints/filtered_depth_from_scratch/base/',
  'root_dir_list': [
      '/usr/project/gen_data/CURRENT_FINAL_OUT/train/BASE/'
    ], # Dir for the training data
  'val_dir_list': [
      '/usr/project/gen_data/CURRENT_FINAL_OUT/val/BASE/'
    ], # Dir for the validation data
  'scale':  59.16626341506611, # scale for Dense Dataset
  'trans' : 0.3867944558791379, # translation for Dense Dataset
...
class DPTFinetune(pl.LightningModule):
  def __init__(self):
    ...
    self.criterion_masked_ssi = ScaleInvariantLoss()
  ...
  def training_step(self, train_batch, train_idx):
    ...
    if torch.count_nonzero(mask) == 0:
      out = torch.clip(out, 1e-8)
      mask = (depth_img > 0) & (out > 0)

    #out = torch.clip(out, min=1e-8)
    ...
    if train_idx % 500 == 0:
      ...
      import time
      ti = time.time()
      plt.savefig("debug_outputs/epoch_base"+str(ti)+"_"+str(train_idx)+".jpg")
      plt.close()
  ...
  def run_kitti_test(self):
    ...
    ds = KittiDepthSelectionV2("../../KITTI_filtered/eigen_test/", "../../KITTI/", transform)
    dl = DataLoader(
    ...
    self.log('val/abs_rel', overall_metrics[0])
    self.log('val/sqr_rel', overall_metrics[1])
    self.log('val/rmse', overall_metrics[2])
    self.log('val/rmse_log', overall_metrics[3])
    self.log('val/silog', overall_metrics[4])
    self.log('val/a1', overall_metrics[5])
    self.log('val/a2', overall_metrics[6])
    self.log('val/a3', overall_metrics[7])
  ...
  def validation_step(self, val_batch, batch_idx):
    ...
    if batch_idx == 0:
      ...
      import time
      ti = time.time()
      plt.savefig("debug_outputs/epoch_val_base"+str(ti)+"_"+str(batch_idx)+".jpg")
      plt.close()
...
trainer = pl.Trainer(max_epochs=params['num_epochs'], callbacks=[lr_monitor], accelerator='gpu', default_root_dir=params['save_dir'])
